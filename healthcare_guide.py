...
# Load datasets
sym_des = pd.read_csv(r"C:\Users\SHAMBHAVI MISHRA\PycharmProjects\PDF-Q\IntelliAid\symtoms_df.csv")
precautions = pd.read_csv(r"C:\Users\SHAMBHAVI MISHRA\PycharmProjects\PDF-Q\IntelliAid\precautions_df.csv")
workout = pd.read_csv(r"C:\Users\SHAMBHAVI MISHRA\PycharmProjects\PDF-Q\IntelliAid\workout_df.csv")
description = pd.read_csv(r"C:\Users\SHAMBHAVI MISHRA\PycharmProjects\PDF-Q\IntelliAid\description.csv")
medications = pd.read_csv(r"C:\Users\SHAMBHAVI MISHRA\PycharmProjects\PDF-Q\IntelliAid\medications.csv")
diets = pd.read_csv(r"C:\Users\SHAMBHAVI MISHRA\PycharmProjects\PDF-Q\IntelliAid\diets.csv")
...
def helper(dis):
    ...
    med = [m for m in med if pd.notna(m)]

    die = diets[diets['Disease'] == dis]['Diet'].values
    die = [d for d in die if pd.notna(d)]

    wrkout = workout[workout['disease'] == dis]['workout'].values
    wrkout = [w for w in wrkout if pd.notna(w)]
...
def healthcare_guide_app():
    ...
    st.markdown("""
        ...
        **Instructions:**
        - Enter your symptoms in the text box below, separated by commas (e.g., itching, skin rash).
        - Click on the 'Predict' button to get a prediction of the possible disease.
        - Once the prediction is made, you can explore additional information about the disease by clicking on the buttons below.
        - The model will provide descriptions, precautions, medications, recommended diet, and workout suggestions based on the predicted disease.

        **Note:** This system is for informational purposes only. Always consult a healthcare professional for accurate diagnosis and treatment.
    """)
    ...
    if st.button("Predict"):
        if symptoms:
            ...
            st.markdown(f"<div class='info-box'><h4>Predicted Disease: {predicted_disease}</h4></div>", unsafe_allow_html=True)
            st.markdown(f"<div class='info-box'><h4>Description:</h4><p>{dis_des}</p></div>", unsafe_allow_html=True)

            st.markdown(f"<div class='info-box'><h4>Precautions:</h4><ul>{''.join([f'<li>{i}</li>' for i in precautions])}</ul></div>", unsafe_allow_html=True)
            st.markdown(f"<div class='info-box'><h4>Medications:</h4><ul>{''.join([f'<li>{med}</li>' for med in medications])}</ul></div>", unsafe_allow_html=True)
            st.markdown(f"<div class='info-box'><h4>Recommended Diet:</h4><ul>{''.join([f'<li>{diet}</li>' for diet in rec_diet])}</ul></div>", unsafe_allow_html=True)
            st.markdown(f"<div class='info-box'><h4>Workout:</h4><ul>{''.join([f'<li>{exercise}</li>' for exercise in workout])}</ul></div>", unsafe_allow_html=True)
        else:
            st.error("Please enter symptoms to predict.")

    st.markdown("<div class='footer'>Consult a doctor for more accurate and reliable recommendations.</div>", unsafe_allow_html=True)
